...
import com.timebill.stopwatch.utils.PdfUtils
...
class SessionDetailsActivity : AppCompatActivity() {
    ...
    private fun observeEmailStatus() {
        viewModel.emailStatus.observe(this) { status ->
            when (status) {
                is SessionDetailsViewModel.EmailStatus.Loading -> {
                    // Handled in processSendInvoice
                }
                is SessionDetailsViewModel.EmailStatus.Success -> {
                    hideLoadingDialog()
                    showSuccessDialog()
                    viewModel.resetEmailStatus()
                }
                is SessionDetailsViewModel.EmailStatus.Error -> {
                    hideLoadingDialog()
    ...
    private fun validateInvoiceData(): Boolean {
        val clientName = binding.etClientName.text.toString().trim()
        val hasDetails = binding.cbMoreDetails.isChecked
        val clientEmail = if (hasDetails) binding.etClientEmailSD.text.toString().trim() else ""
        ...
        if (!hasDetails || clientEmail.isEmpty()) {
            Toast.makeText(this, "Client Email is required. Please check 'Add More Details'.", Toast.LENGTH_LONG).show()
            return false
        }
        ...
        if (workName.isEmpty()) {
            Toast.makeText(this, "Work Name is required", Toast.LENGTH_SHORT).show()
            return false
        }
        if (profile == null || profile.fullName.isNullOrEmpty() || profile.email.isNullOrEmpty()) {
            Toast.makeText(this, "Please complete your Business Profile first", Toast.LENGTH_LONG).show()
            return false
        }
        return true
    }

    private fun showSendInvoiceConfirmation() {
        val clientName = binding.etClientName.text.toString().trim()
        val clientEmail = binding.etClientEmailSD.text.toString().trim()
    ...
    private fun processSendInvoice() {
        showLoadingDialog("Generating invoice and sending email...")

        val session = currentSession ?: return
        val profile = userProfile ?: UserProfile()

        // Capture UI values and update Firebase
        ...
        val invoiceNum = if (!session.invoiceNumber.isNullOrEmpty()) {
            session.invoiceNumber!!
        } else if (!session.receiptNumber.isNullOrEmpty()) {
        ...
        if (pdfFile.exists()) {
            viewModel.sendInvoiceEmail(updatedSession, profile, pdfFile)
        } else {
            hideLoadingDialog()
            showErrorDialog("Failed to generate or locate invoice PDF.")
        }
    }
    ...
    private fun showSuccessDialog() {
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setMessage("The invoice has been emailed successfully to $clientEmail.")
    ...
    private fun populateSessionData(session: Session) {
        ...
        binding.etDuration.setText(AppUtils.formatDuration(session.durationMillis ?: 0L))
    ...
    private fun generateAndHandlePdf(action: Action) {
        ...
        val pdfDocument = PdfUtils.createPdfDocument(sessionForPdf, profile)
